...
pub trait Collatz: Unsigned + Integer + PrimInt + From<u8> {}
...
pub trait Steps: Sized {
    ...
    fn steps_to_one(self) -> Option<u64>;

    /// Returns how many steps a number takes to decrease,
    /// or None if there is an overflow
    fn steps_to_decrease(self) -> Option<u64>;
    ...
    fn steps_to_one_for_odd_number(self) -> Option<u64>;
}
...
pub trait Transformations: Sized {
    /// Returns every number that this number becomes as it falls to one
    fn transformations_to_one(self) -> Option<Vec<Self>>;
}
